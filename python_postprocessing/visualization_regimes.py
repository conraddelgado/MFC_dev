...
for i in [N//2]: #range(0, N, N//10):
    fig, ax = plt.subplots()
    plt.contourf(X, Y, q_filtered[:, :, i, -1], levels=num_levels)
    plt.title(r'Numerical Filtered Fluid Volume Fraction, $\sigma=%sd_p$' % str(3/2))
    plt.colorbar(label=r'$\phi$')
    plt.xlabel('x')
    plt.ylabel('y')
    ax.add_patch(sphere_outline)
    plt.tight_layout()
    plt.savefig(f'numerical_volfrac_3', dpi=300)
    plt.show()
    plt.close()
